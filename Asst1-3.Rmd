...
**Due Sunday, February 16 @ midnight through Github**
...
5. Include an updated final logistic regression model to address question the question and interpret the results. Remember to use the function `glm` as shown in the slides from today's class.
...
First, use logistic regression to estimate propensity scores for the treatment of smoking (`eversmk`); that is, use logistic regression to estimate the probability of smoking given possible confounders.
...
Then, using these propensity scores, use logistic regression with quintiles of your propensity scores to answer the question of interest.
...
* In the window that opens, **stage** your changes by clicking the check boxes next to the `Asst1-3.Rmd` and `Asst1-3.html` files.
